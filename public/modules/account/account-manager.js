...
async function showAdmin() {
  ...
  const body = document.getElementById('accountPanelBody');
  body.innerHTML = `<button class="btn btn-ghost" data-account-action="back">← Tài khoản</button><h3>Độ tin cậy hệ thống</h3><div class="account-stats"><div><strong>${Number(data.cache.entries) || 0}</strong><span>Cache entries</span></div><div><strong>${data.cronRuns.length}</strong><span>Cron runs</span></div><div><strong>${data.retryJobs.filter((j) => j.status === 'pending').length}</strong><span>Retry pending</span></div></div><section class="account-log"><h3>Dịch vụ</h3>${data.services.map((service) => `<div><span>${escapeHtml(service.service)}</span><strong class="${service.status === 'operational' ? 'sent' : 'failed'}">${escapeHtml(service.status)}</strong><time>${escapeHtml(new Date(service.checked_at).toLocaleString('vi-VN'))}</time></div>`).join('') || '<p>Chưa có lần kiểm tra nào.</p>'}</section><section class="account-log"><h3>Cron gần đây</h3>${data.cronRuns.slice(0, 10).map((run) => `<div><span>${escapeHtml(run.job_name)} · ${Number(run.checked_count) || 0} game</span><strong class="${run.status === 'success' ? 'sent' : 'failed'}">${escapeHtml(run.status)}</strong><time>${escapeHtml(new Date(run.started_at).toLocaleString('vi-VN'))}</time></div>`).join('') || '<p>Chưa có lần chạy.</p>'}</section>`;
}
